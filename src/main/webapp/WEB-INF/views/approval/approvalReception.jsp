				...
				<table class="table table-hover">
					...
					<c:forEach var="a" items="${list}">
						<tr>
							...
							<td>${a.aName }</td>
							<td>
							<c:url var="ardetail" value="aRequestdetail.do?button=2">
					...
					<c:if test="${empty list}">
						<tr><td colspan="8" style="text-align: center"><br>결재한 문서가 없습니다</td></tr>
					</c:if>
					</tbody>
				</table>
				</div>
					<div class="page_wrap">
						<div class="page_nation">
							<!-- << 버튼 눌렀을 때 1페이지로 이동 -->
							<c:url var="first" value="aReceptionList.do">
							...
							<c:if test="${ pi.currentPage > 1 }">
								<c:url var="before" value="aReceptionList.do">
							...
							<c:forEach var="p" begin="${ pi.startPage }"
								...
								<c:if test="${ p ne pi.currentPage }">
									<c:url var="pagination" value="aReceptionList.do">
							...
							<c:if test="${ pi.currentPage < pi.maxPage }">
								<c:url var="after" value="aReceptionList.do">
									<c:param name="page" value="${ pi.currentPage + 1 }" />
								</c:url>
							...
							<!-- 마지막 페이지로 -->
							<c:url var="last" value="aReceptionList.do">
