...
<jsp:include page="../common/menubar.jsp"/>
	...
	<input type="hidden" name="aName" value="증명서발급">
		<div class="content1">
			<div id="buttongroup">
				<button class="btn1" id="btn1" type="button">
					<img class="icon1" src="resources/images/approval/edit.png">결재요청
				</button>
				<button type="submit"class="btn1" id="btn2" onclick="javascript: form.action='certificateInsert.do?aStatus=S'">
							...
							<tbody>
								<tr>
									...
									<td
										...
										colspan="2">
										<input class="ipt_editor ipt_editor_date" size="30px" type="text" name="cKind" value="${c.cKind}">
									...
									<td
										style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 14px; font-weight: normal; vertical-align: middle;"
										colspan="2">
										<c:if test="${c.cNum eq null }">
											<input class="ipt_editor ipt_editor_currency" type="number" name="cNum" value="1">
										</c:if>
										<c:if test="${c.cNum ne null }">
											<input class="ipt_editor ipt_editor_currency" type="number" name="cNum" value="${c.cNum}">
										</c:if>
									</td>
								...
								<tr>
									...
									<td
										...
										colspan="4"><textarea rows="3" cols="80" class="ipt_editor" type="text" name="cPurpose" style="resize:none">${c.cPurpose }</textarea><br></td>
								</tr>
								<tr>
									<td
										style="background: rgb(221, 221, 221); padding: 5px; border: 1px solid black; border-image: none; height: 25px; text-align: center; color: rgb(0, 0, 0); font-size: 14px; font-weight: bold; vertical-align: middle;">
									...
									<td
										style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 14px; font-weight: normal; vertical-align: middle;"
										><input class="ipt_editor ipt_editor_currency" type="text" name="cSubmission" value="${c.cSubmission }"></td>
									<td
										style="background: rgb(221, 221, 221); padding: 5px; border: 1px solid black; border-image: none; height: 25px; text-align: center; color: rgb(0, 0, 0); font-size: 14px; font-weight: bold; vertical-align: middle;">
		...
		<div class="content2">
			<div id="radiobox">
				...
				<div id="approvalinfo"><c:forEach var="l" items="${llist}"><c:if test="${l.lLevel ne 0 }"><div class="peopleinfo"><div id="line"><c:forEach var="e" items="${elist }"><c:if test="${l.leId eq e.eId }"><img class="pimg" src='resources/images/profileUploadFiles/${l.changeName }'></c:if><c:if test="${l.leId ne e.eId }"><img src='resources/images/common/profile_none.png'></c:if></c:forEach><span class="name1"><span calss="ename">${l.eName}${l.rName}</span><p class="deptname">${l.dName }</p><button type="button" class="dBtn" id="deleteImg"><input type="hidden" name="leId" class="user1" value=${ l.leId }><img id='dbtn' src='resources/images/approval/trash.png'></button><input type="hidden" name="lLevel" value="결재"><span class="status">결재</span></span></div></div></c:if></c:forEach></div>
			...
			<c:if test="${l.lLevel eq 0 }">
				<div class="peopleinfo">
					...
					<c:forEach var="e" items="${elist }">
						<c:if test="${l.leId eq e.eId }">
							<img class="pimg" src='resources/images/profileUploadFiles/${e.changeName }'>
						</c:if>
						<c:if test="${l.leId ne e.eId }">
	...
	<script>
        ...
        window.onload = function(){
        	if($("select[name=cSelect]").val() == ""){
        		$("select[name=cSelect]").val("국문").attr("selected", "selected");
        	}else
        		$("select[name=cSelect]").val("${c.cSelect}").attr("selected", "selected");
   		}
    </script>
</body>
