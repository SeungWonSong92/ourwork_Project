		...
		<div class="section2">
            <div id="test" align="right">
                ...
                <form name="searchForm" method="get">
					...
					<button type="submit" class="btn btn-secondary btn-sm" onclick="javascript: form.action='searchF.do'">검색</button>
					<button type="button" onclick="sdelete();" class="btn btn-secondary btn-sm">선택삭제</button>
				</form>
            </div>
            <div id="mainContent">
                <div id="messageList">
								...
								<tr>
									...
									<td id="mContent">
										<c:if test="${ !empty e.mTitle }">
											<a href="${ sendEmailDetailView }" id="cp">${ e.mTitle }</a>
										</c:if>
										<c:if test="${ empty e.mTitle }">
											<a href="${ sendEmailDetailView }" id="cp">(제목 없음)</a>
										</c:if>
									</td>
                ...
                <div class="page_wrap">
					<div class="page_nation">
						<!-- << 버튼 눌렀을 때 1페이지로 이동 -->
		                	<c:url var="first" value="sendMailList.do">
						...
						<!-- 마지막 페이지로 -->
		                    <c:url var="last" value="sendMailList.do">
				...
				<c:if test="${ se ne null }">
					<div class="page_wrap">
						<div class="page_nation">
							<!-- << 버튼 눌렀을 때 1페이지로 이동 -->
		                	<c:url var="first" value="searchF.do">
		                		<c:param name="page" value="${ 1 }"/>
							...
							<c:if test="${ pi.currentPage > 1 }">
								<c:url var="before" value="searchF.do">
									<c:param name="page" value="${ pi.currentPage - 1 }"/>
									<c:param name="searchValue" value="${ se.searchValue }"/>
								</c:url>
								<a class="arrow prev" href="${ before }">&lt;</a>
							...
							<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
								...
								<c:if test="${ p ne pi.currentPage }">
									<c:url var="pagination" value="searchF.do">
										<c:param name="page" value="${ p }"/>
										<c:param name="searchValue" value="${ se.searchValue }"/>
									</c:url>
									<a href="${ pagination }">${ p }</a> &nbsp;
							...
							<c:if test="${ pi.currentPage < pi.maxPage }">
								<c:url var="after" value="searchF.do">
									<c:param name="page" value="${ pi.currentPage + 1 }"/>
									<c:param name="searchValue" value="${ se.searchValue }"/>
								</c:url>
								<a class="arrow next" href="${ after }">&gt;</a>
							...
							<!-- 마지막 페이지로 -->
		                    <c:url var="last" value="searchF.do">
		                    	<c:param name="page" value="${ pi.maxPage }"/>
		                    	<c:param name="searchValue" value="${ se.searchValue }"/>
		                    </c:url>
		                    <a class="arrow nnext" href="${ last }">&gt;&gt;</a>
		...
		function sdelete(){
		      var check ="";
		       ...
		       if(check == ""){
		          ...
		          location.href = "sEmailDelete.do?check="+ check;
		      }
